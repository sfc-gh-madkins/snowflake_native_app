...
dataset_ref = model_ref.list_artifacts().where(col('TYPE') == 'DATASET').select('NAME', 'VERSION').collect()[0]
...
with st.form("entity_form"):
    ...
    for entity in range(entity_join_keys):
    ...
    if submitted:
        ...
        enity_df = pd.DataFrame([input_values])
        st.write(df)
